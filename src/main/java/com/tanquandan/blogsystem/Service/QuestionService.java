...
public class QuestionService {
    ...
    public PaginationDTO<QuestionDTO> listAllQuestions(int currentPage, int size) {
        ...
        int questionsAmount = selectAllQuestionsCount();

        List<QuestionDTO> questions = setQuestions(tmp_questions);
        PaginationDTO<QuestionDTO> paginationDTO = new PaginationDTO(questions, currentPage, size, questionsAmount);
    ...
    public PaginationDTO<QuestionDTO> listSearchQuestions(int currentPage, int size, String search) {
        int offset = (currentPage - 1) * size;
        // 1.处理Search: Search 不能为空
        if (StringUtils.isEmpty(search)) {
            PaginationDTO<QuestionDTO> paginationDTO = new PaginationDTO<>(null, currentPage, size, 0L);
            paginationDTO.setPagination();
            return paginationDTO;
        }
        List<String> keywordsArr = Arrays.stream(search.split(" "))
                .map(words -> words.replace("+", " ")
                        .replace("?", " ")
                        .replace("*", " "))
                .filter(words -> !words.equals(""))
                .collect(Collectors.toList());
        String keywords = String.join("|", keywordsArr);
        // 2.根据Search检索数据库
        List<Question> tmp_questions = questionExtMapper.selectSearchQuestions(keywords, offset, size);
        int questionsAmount = questionExtMapper.selectSearchQuestionsCount(keywords, offset, size);
    ...
    // 10.用户查看某个问题时，侧栏会展示tag相同的其他问题
    public PaginationDTO<QuestionDTO> queryAllRelatedQuestions(String tagId, int currentPage, int size) {
        int offset = currentPage * size - 1;
        List<Question> tmp_questions = questionExtMapper.selectAllRelatedQuestions(tagId, offset, size);
        int questionsAmount = questionExtMapper.selectAllRelatedQuestionsCount(tagId);
        List<QuestionDTO> questions = setQuestions(tmp_questions);
        PaginationDTO<QuestionDTO> paginationDTO = new PaginationDTO(questions, currentPage, size, questionsAmount);
        paginationDTO.setPagination();
        return paginationDTO;
